...
(() => {
  ...
  async function iniciarPanel() {
    ...
    try {
      ...
      if (sessionError || !(session == null ? void 0 : session.user)) {
  ...
  async function obtenerPerfilAdministrador(userId) {
    const { data, error } = await supabaseClient.from("usuarios").select("id, nombre, apellido, rol, estado, activo").eq("id", userId).maybeSingle();
    if (error || !data) {
    ...
    const autorizado = (rol === "administrador" || rol === "admin") && estado === "aprobado" && data.activo !== false;
  ...
  async function cargarDatos() {
    ...
    try {
      const [usuariosResultado, empresasResultado, conveniosResultado] = await Promise.all([
        supabaseClient.from("usuarios").select("id,nombre,apellido,dni,telefono,mail,rol,empresa,convenio,estado,activo,created_at,updated_at").order("created_at", { ascending: false }),
        supabaseClient.from("empresas").select("id,nombre,activa").order("nombre"),
        supabaseClient.from("convenios").select("id,nombre,activo").order("nombre")
      ]);
  ...
  function renderizarTablaUsuarios() {
    ...
    const filtrados = users.filter((usuario) => {
      ...
      return (!busqueda || textoUsuario.includes(busqueda)) && (estadoSeleccionado === "todos" || normalizar(usuario.estado) === estadoSeleccionado) && (rolSeleccionado === "todos" || normalizar(usuario.rol) === rolSeleccionado);
    });
  ...
  async function alternarEstadoUsuario(userId) {
    ...
    const confirmar = window.confirm(
      reactivar ? `¿Deseás reactivar a ${obtenerNombreCompleto(usuario)}?` : `¿Deseás bloquear a ${obtenerNombreCompleto(usuario)}?`
    );
    if (!confirmar) return;
    mostrarCarga(true);
    try {
  ...
  function establecerBotonCargando(button, cargando) {
    ...
    button.innerHTML = cargando ? '<i class="fa-solid fa-spinner fa-spin"></i> Guardando...' : button.dataset.originalHtml;
  }
  ...
  function escaparHtml(valor) {
    ...
    elemento.textContent = String(valor != null ? valor : "");
